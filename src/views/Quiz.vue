  ...
  <div class="container-fluid">
      ...
      <div class='d-flex' v-if="submitted">
        <div class="card w-25 mx-5 mt-5">
            ...
            <ol class="mt-2" >
                <li class="text-start fw-bold" v-for="answer in answers" v-bind:key="answer">{{answer}}</li>
            </ol>
        </div>
        <div class="card w-25 mx-5 mt-5" v-if="results.pass">
            <h5 class="card-header bg-success text-white">Score</h5>
            <h5 class="card-body">{{results.correct}}/{{results.total}} - PASSED</h5>
        </div>
        <div class="card w-25 mx-5 mt-5" v-if="!results.pass">
            <h5 class="card-header bg-danger text-white">Score</h5>
            <h5 class="card-body">{{results.correct}}/{{results.total}} - FAILED</h5>
        </div>
      </div>
      <button class="mt-5 btn btn-primary" :disabled="results.pass" @click="submitAnswer()">Submit</button>
