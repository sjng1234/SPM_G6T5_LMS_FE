<template>
  <div class="container m-0 p-0">
    ...
    <form class="container-fluid p-5" onsubmit="return false;">
      ...
      <button type="button" v-on:click="addQn()" class="btn btn-success mx-2">
        + Add Question
      </button>
      <button type="button" v-on:click="deleteQn()" class="btn btn-danger mx-2">
        - Delete Question
      </button>

      <div
        class="questions container"
        v-for="(qn, index) in question"
        v-bind:key="qn"
      >
        <div class="input-group mb-3 mt-2">
          <span class="input-group-text" id="basic-addon1"
            >Question {{ qn.question_id }}</span
          >
          <input
            type="text"
            class="form-control"
            v-model="question[index].question_description"
            required
          />
          <button
            type="button"
            class="btn btn-primary"
            v-on:click="addOption(index)"
          >
            + Option
          </button>
          <button
        ...
        <div
          class="input-group mb-3 w-50 option"
          v-for="(option, o_index) in qn.options"
          v-bind:key="option"
        >
          <div class="input-group-text">
            <input
              ...
              type="radio"
              v-model="qn.options[o_index].is_correct_answer"
              :name="'question' + index"
              :value="true"
          ...
          <input
            type="text"
            class="form-control"
            v-model="qn.options[o_index].option"
            required
