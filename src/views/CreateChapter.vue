    ...
    <div>
        ...
        <form class="container-fluid p-5" onsubmit="return false;">
            <div class="input-group mb-3 mt-2">
                ...
                <input type="number" class="form-control" style="text-transform:uppercase" v-model="chapter_id" required>
            </div>
            <div class="input-group mb-3 mt-2">
                <span class="input-group-text" >Chapter Name</span>
                <input type="text" class="form-control"  v-model="chapter_name" required>
            </div>
            ...
            <div class="input-group mb-3 mt-2">
                ...
                <input type="text" class="form-control"  v-model="material_name" required>
            </div>  
            <div class="input-group mb-3 mt-2">
                <input type="file" class="form-control" accept=".zip,.rar,.7zip" required>
            </div>

            <button type="submit" onsubmit="return false;" v-on:click="createChapter()" class="btn btn-primary">Submit</button>
        </form>
    </div>
    ...
    methods:{
        createChapter(){
            if(this.chapter_id && this.chapter_name && this.material_name){
                ...
                axios.post('http://127.0.0.1:5000/chapter/addChapter', chapter_data).then(async (response)=>{
                    ...
                    if(this.material_name){
                        ...
                        axios.put('http://127.0.0.1:5000/material/add', material_data).then(()=>{console.log('success')}).catch((error) => {console.log(error)})
                        }
                    this.uploaded = true
                    setTimeout(()=>{
                        this.$router.go(-1);
